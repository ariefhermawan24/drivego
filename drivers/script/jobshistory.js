        ...
        if (snapshot.exists()) {
          ...
          if (keyToDelete) {
            ...
            remove(transaksiToDeleteRef)
              .then(() => {
                showToast('Transaksi berhasil dihapus!', 'success', 'fas fa-check-circle');
              })
              .catch((error) => {
                console.error('Error saat menghapus transaksi:', error);
                showToast('Gagal menghapus transaksi.', 'danger', 'fas fa-times-circle');
              });
          } else {
            showToast('Transaksi tidak ditemukan.', 'warning', 'fas fa-info-circle');
          }
        } else {
          showToast('Tidak ada data transaksi.', 'warning', 'fas fa-info-circle');
        }
      }).catch((error) => {
        console.error('Error saat mengambil data transaksi:', error);
        showToast('Terjadi kesalahan saat mengambil data.', 'danger', 'fas fa-times-circle');
      });
    }
  );
}
